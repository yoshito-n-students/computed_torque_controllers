...
private:
  struct ControlledJointInfo {
    ...
    boost::shared_ptr< rt::RealtimeBuffer< double > > vel_cmd_buf;
    ros::Subscriber vel_cmd_sub;
    boost::optional< jli::VelocityJointSaturationHandle > vel_cmd_sat_handle;
...
public:
  ...
  virtual bool init(hi::RobotHW *hw, ros::NodeHandle &root_nh, ros::NodeHandle &controller_nh) {
    ...
    BOOST_FOREACH (const JointNamespaceMap::value_type &ctl_joint_ns, ctl_joint_namespaces) {
      ...
      info.vel_cmd_buf.reset(new rt::RealtimeBuffer< double >());
      info.vel_cmd_sub = controller_nh.subscribe< std_msgs::Float64 >(
          rn::append(name, "command"), 1,
          boost::bind(&VelocityJointController::velocityCommandCB, _1, info.vel_cmd_buf));

      // satulation based on limits (optional)
      jli::JointLimits limits;
      if (jli::getJointLimits(robot_desc.getJoint(name), limits)) {
        const hi::JointHandle vel_cmd_handle(info.hw_state_handle, &info.vel_cmd);
        info.vel_cmd_sat_handle = jli::VelocityJointSaturationHandle(vel_cmd_handle, limits);
      }
    }
  ...
  virtual void starting(const ros::Time &time) {
    ...
    BOOST_FOREACH (ControlledJointInfoMap::value_type &ctl_joint, ctl_joints_) {
      ...
      info.vel_cmd_buf->writeFromNonRT(0.);
  ...
  virtual void update(const ros::Time &time, const ros::Duration &period) {
    // calc pos/vel/acc setpoints from subscribed velocity commands
    ...
    BOOST_FOREACH (ControlledJointInfoMap::value_type &joint, ctl_joints_) {
      ...
      info.vel_cmd = *info.vel_cmd_buf->readFromRT();
      // satualate the command. below may modify info.vel_cmd according to the joint limit
      if (info.vel_cmd_sat_handle) {
        info.vel_cmd_sat_handle->enforceLimits(period);
      }
      // setpoints as derivatives of (saturated) velocity command
      PosVel &sp(ctl_joint_setpoints[name]);
      const double dt(period.toSec());
      sp.pos = info.prev_pos_sp + info.vel_cmd * dt;
      sp.vel = info.vel_cmd;
...
private:
  static void velocityCommandCB(const std_msgs::Float64ConstPtr &msg,
